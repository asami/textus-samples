# Sample Checklists

This directory stores one progress tracking document per sample.

Each sample document is intended to be updated independently:

- implementation progress
- execution verification
- README completion
- sample-specific exit criteria

Current sample documents:

- `01-minimal.md`
- `01.a-invocation-source-lab.md`
- `01.b-startup-shapes-lab.md`
- `01.c-builtin-and-help-lab.md`
- `01.d-component-script.md`
- `02-crud.md`
- `02.a-crud-seed-import-lab.md`
- `02.b-simpleentity-crud-lab.md`
- `02.c-crud-sqlite-lab.md`
- `02.d-crud-server-memory-lab.md`
- `02.e-crud-explicit-sync-lab.md`
- `02.f-crud-nested-value-lab.md`
- `03-operation.md`
- `03.a-operation-command-lab.md`
- `03.b-operation-entity-lab.md`
- `04-cqrs.md`
- `04.a-designed-sync-command-lab.md`
- `04.b-test-sync-command-lab.md`
- `05-event-driven.md`
- `05.a-event-job-trace-lab.md`
- `05.b-event-job-server-client-lab.md`
- `06-job.md`
- `06.a-job-control-lab.md`
- `06.b-job-control-demo-lab.md`
- `07-aggregate.md`
- `07.a-aggregate-single-record-lab.md`
- `07.b-aggregate-relation-boundary-model.md`
- `07.c-aggregate-external-update-semantics.md`
- `08-view.md`
- `08.a-view-definition-lab.md`
- `09-subsystem.md`
- `10-subsystem-wiring.md`
- `101-distributed.md`
