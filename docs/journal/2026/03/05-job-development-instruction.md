# 05-job Development Instruction
...
The point of `05-job` is to establish the job-management surface first.

## Why This Sample Exists

`03-cqrs` already shows a job-backed command.
`04-event-driven` already shows an event-triggered reaction.
...
The first completion line for `05-job` is:
...
- model-driven sample, if the existing `03-cqrs` pattern is enough
- command-first usage
...
That later lab should not be forced into the first `05-job` completion line.

## Deliverables

- `samples/05-job/README.md`
- `docs/phase/samples/05-job.md`
- sample implementation files under `samples/05-job/`
