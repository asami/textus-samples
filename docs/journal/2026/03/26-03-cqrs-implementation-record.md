# 03-cqrs Implementation Record

## Summary

`03-cqrs` now demonstrates a visible split between a job-backed command side and an immediate query side.

## Facts

- The sample is model-driven via `samples/03-cqrs/src/main/cozy/cqrs.cml`.
...
- A seeded `Item` record is provided under `samples/03-cqrs/entity.d/item.yaml`.
