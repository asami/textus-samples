...
final class BetaComponent extends Component {
  override def subsystemDefinitionRecords: Vector[Record] =
    Vector(Record.data("name" -> "testsubsystemmulti"))
}

object BetaComponent extends Component.Factory {
  val name = "betacomp"
  val componentId = ComponentId(name)

  protected def create_Components(params: ComponentCreate): Vector[Component] =
    Vector(BetaComponent())
...
final case class HelloActionCall(
  ...
  override def execute(): Consequence[OperationResponse] =
    response_string("Hello from betacomp in testsubsystemmulti")
}
