...
This sample is the practical server/client follow-up to `05-event-driven` and `05.a-event-job-trace-lab`.
...
`05-event-driven` showed the base event surface.

`05.a-event-job-trace-lab` showed how the same reaction appears in `job-control`.

`05.b-event-job-server-client-lab` turns that into the practical local shape that a user actually runs:
