...
`06.a-job-control-lab` is the control-oriented companion to `06-job`.

- [06-job] focuses on observing submitted jobs
- `06.a-job-control-lab` focuses on controlling a running job
...
```bash
$ cd samples/06.a-job-control-lab
$ ../../bin/setup cozy
```
...
- `cncf`
  ...
  - in this repo it is invoked as `../../bin/cncf`
- `--discover=classes`
...
```bash
$ bash ../../bin/cncf --discover=classes client job-control-lab.item.create-item --name quick --title Quick
```
...
```bash
$ bash ../../bin/cncf --discover=classes client job-control.job-admin.suspend-job --id cncf-job-job-1775523947840-3JgfELQBaYtc3yhMeTZs76 --privilege content_admin
```
...
```bash
$ bash ../../bin/cncf --discover=classes client job-control.job.get-job-status --id cncf-job-job-1775523947840-3JgfELQBaYtc3yhMeTZs76
```
...
```bash
$ bash ../../bin/cncf --discover=classes client job-control.job-admin.resume-job --id cncf-job-job-1775523947840-3JgfELQBaYtc3yhMeTZs76 --privilege content_admin
```
...
```bash
$ bash ../../bin/cncf --discover=classes client job-control.job.await-job-result --id cncf-job-job-1775523947840-3JgfELQBaYtc3yhMeTZs76
```
...
```bash
$ bash ../../bin/cncf --discover=classes client job-control.job.load-job-history --id cncf-job-job-1775523947840-3JgfELQBaYtc3yhMeTZs76
```
...
```bash
$ bash ../../bin/cncf --discover=classes command event.event-admin.load-job-events --id cncf-job-job-1775523947840-3JgfELQBaYtc3yhMeTZs76 --privilege content_admin
```
...
```bash
$ bash ../../bin/cncf --discover=classes client job-control-lab.item.create-item --name cancel --title Cancel
$ bash ../../bin/cncf --discover=classes client job-control.job-admin.cancel-job --id cncf-job-job-1775524097115-1otiOMA68eDfhpzSal9CAi --privilege content_admin
```
...
- [06-job] focuses on job observation
  ...
  - get-status
  - load-history
- `06.a-job-control-lab` focuses on job control
  - suspend
  - resume
  - cancel
