...
This lab explains the single-record aggregate pattern as a concrete companion to [06-aggregate](/Users/asami/src/dev2026/cncf-samples/samples/06-aggregate/README.md).

The main line in `06-aggregate` is application-join:
...
That is different from the main `06-aggregate` sample, where `OrderLine` is a separate entity and the framework builds the aggregate by joining persisted members.
